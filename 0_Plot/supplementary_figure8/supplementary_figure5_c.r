pacman::p_load(tidyverse, readxl, vcfR, VariantAnnotation, rtracklayer, Biostrings, Rsamtools, patchwork, plyranges, genomation, dplyr, ggsignif)
...
## get all the enhancer sites
enhancer_files = system("ls enhancer.bed",intern = TRUE)
length(enhancer_files)

enhancer = lapply(enhancer_files, function(x){
    tmp <- read.table(x,sep = "\t", header = FALSE)
    tmp <- GRanges(seqnames = tmp[, 1],ranges = IRanges(start = tmp[, 2], end = tmp[, 3]))
    seqlevelsStyle(tmp) <- "UCSC"
    return(tmp)
})

enhancer = keepSeqlevels(GRangesList(enhancer), c(paste0("chr",1:22),"chrX","chrY"), pruning.mode = "tidy")
enhancer_final = do.call("c",enhancer)
enhancer_final = enhancer_final[!duplicated(enhancer_final)]

## using the generic enhancer regions
peak_plot <- function(){
    enhancerbs_regions = enhancer_final
    enhancerbs_regions = enhancerbs_regions %>% reduce_ranges()
    enhancerbs_regions_center <- resize(enhancerbs_regions, width=1, fix="center")
    enhancerbs_regions_upstream <- resize(enhancerbs_regions_center, width=5000, fix="end")
    enhancerbs_regions_downstream <- resize(enhancerbs_regions_center, width=5000, fix="start")

    bin_num = 5
    x1 = suppressWarnings(ScoreMatrixList(c(shared=granges(v_shared),unique=granges(v_unique)), bin.num=bin_num, bin.op = "max", windows=enhancerbs_regions_upstream,strand.aware=TRUE))
    x2 = suppressWarnings(ScoreMatrixList(c(shared=granges(v_shared),unique=granges(v_unique)), bin.num=bin_num, bin.op = "max", windows=enhancerbs_regions_downstream,strand.aware=TRUE))
    ...
    theme_bw() + labs(color="",x= "Distance from enhancer midpoint (bp)",y="Proportion of mutations") +
...
p1 = peak_plot()+geom_signif(xmin = 0.85,xmax = 1.15,annotations = "p = 0.074",y_position = 0.017,textsize = 9,tip_length = 0.02, color="black")
...
peak_plot <- function(){
    ...
    bin_num = 5
    x1 = suppressWarnings(ScoreMatrixList(c(BP=granges(v_BP),PP=granges(v_PP),P=granges(v_P)), bin.num=bin_num, bin.op = "max", windows=enhancerbs_regions_upstream,strand.aware=TRUE))
    x2 = suppressWarnings(ScoreMatrixList(c(BP=granges(v_BP),PP=granges(v_PP),P=granges(v_P)), bin.num=bin_num, bin.op = "max", windows=enhancerbs_regions_downstream,strand.aware=TRUE))
...
p2 = peak_plot()+geom_signif(xmin = 1,xmax = 1.2,annotations = "p = 0.017",y_position = 0.023,textsize = 9,tip_length = 0.02, color="black")
