            ...
            for operation, length in read.cigartuples:
                if operation == 1:  # Insertion
                    if len(ref) < len(alt) and length == len(alt) - len(ref) and pos == start:
                        count += 1
                    elif (len(ref) < len(alt) and start - read.pos - 1 >= 0 and start - read.pos + len(alt) - 1 <= len(read.query_sequence) and read.query_sequence[start - read.pos - 1:start - read.pos + len(alt) - 1].upper() == alt and pos == start):
                        count += 1
